 ...
 */
public class DatacenterSocial extends CloudSimEntity implements Datacenter {
     ...
     * @see #DatacenterSocial(Simulation, List, VmAllocationPolicy, DatacenterStorage)
     */
    public DatacenterSocial(final Simulation simulation, final List<? extends Host> hostList) {
     ...
     * @param simulation The CloudSim instance that represents the simulation the Entity is related to
     * @param vmAllocationPolicy the policy to be used to allocate VMs into hosts
     * @see #DatacenterSocial(Simulation, List, VmAllocationPolicy)
     * @see #DatacenterSocial(Simulation, List, VmAllocationPolicy, DatacenterStorage)
     * @see #addHost(Host)
    ...
    protected long setupHost(final Host host, long nextId) {
        ...
        host.setActive(((SocialHost)host).isActivateOnDatacenterStartup());
    ...
    private boolean processHostEvents(final SimEvent evt) {
        if (evt.getTag() == CloudSimTags.HOST_ADD) {
            ...
            final SocialHost host = (SocialHost)evt.getData();
    ...
    protected void finishVmMigration(final SimEvent evt, final boolean ack) {
        ...
        if(suitability.fully()) {
            ((VmSocial)vm).updateMigrationFinishListeners(targetHost);
    ...
    public boolean equals(final Object object) {
        ...
        final DatacenterSocial that = (DatacenterSocial) object;
