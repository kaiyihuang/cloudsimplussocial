...
public final class SocialMigrationExample1 {
    ...
    private SocialMigrationExample1(){
        ...
        broker = new DatacenterBrokerBestFitSocial(simulation);
    ...
    public Cloudlet createCloudlet(int length, int pesNumber, int social_id, int security_level, User owner) {
        final Cloudlet cloudlet =
            new SocialCloudlet(length, pesNumber, this.createUtilizationModel(0.10,0.9,true), social_id, security_level, owner);
        return cloudlet;
    }
    ...
    private Datacenter createSocialDatacenter(List<User> listOfUsers) {
        ...
        Datacenter dc = new DatacenterSocial(simulation, hostList, allocationPolicy);
    ...
    public Host createHost(int numberOfPes, long mipsByPe, User owner) {
            ...
            SocialHost host = new SocialHost(HOST_RAM, HOST_BW, HOST_STORAGE, peList);
            host.owner = owner;
